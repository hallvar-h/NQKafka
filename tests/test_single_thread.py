...
def run_server(bootstrap_servers):
    server = NQKafkaServer(bootstrap_servers, run_in_process=False)
    # server.server_process = threading.Thread(target=server.start_server, args=(server.ip, server.port,))
    server.start()
...
if __name__ == '__main__':
    ...
    bootstrap_servers = 'localhost:40009'
    ...
    p_consumer = threading.Thread(target=run_consumer, args=(bootstrap_servers, n_msgs,))
    p_consumer.start()

    p_producer = threading.Thread(target=run_producer, args=(bootstrap_servers, n_msgs,))
    p_producer.start()
